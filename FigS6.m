...
roi = {'DRN_sub_PAG_edit_sub_SC_edit_ero','HB','SN_Pauli'};
seed_roi = {'none','none','none'};
...
%% 2. Run GLM
cond = 'monitoring_firstForages_constant';
...
for r = 1:length(roi)
    ...
    for a = 1:length(subjects)
        ...
        % Run time course
        REG.time = subj_firstForagesData.('onset');
        REG.constant = ones (length (REG.time),1);
        REG.reward = subj_firstForagesData.('reward');
        REG.proximity = subj_firstForagesData.('proximity');
...
for r = 1:numel(roi)
    ...
    %         end
            ...
            for s = 1:length(numsession) % iterate through participants
                ...
                window  = mean(squeeze(allBeta.(sprintf('%s',feat_name)).(sprintf('%s',roi{r},'_',num2str(r))).(sprintf('%s',cond))(reg,:,numsession)),2); %avg across all timepoints for this participant/condition
    
                [m,i]  = max(abs(window(start_ind:end_ind)));
                i      = i + (start_ind-1);
                peak_indices = [peak_indices, i];
                val_tzero = [val_tzero, allBeta.(sprintf('%s',feat_name)).(sprintf('%s',roi{r},'_',num2str(r))).(sprintf('%s',cond))(reg,22,s)]; %index 22 is the sample corresponding to zero
                peak(s,r) = allBeta.(sprintf('%s',feat_name)).(sprintf('%s',roi{r},'_',num2str(r))).(sprintf('%s',cond))(reg,i,s);
            ...
            if makeplot==1
                ...
                beta = squeeze(allBeta.(sprintf('%s',feat_name)).(sprintf('%s',roi{r},'_',num2str(r))).(sprintf('%s',cond))(reg,:,:));
            ...
            Peak_search_window = {strcat([num2str(round(time(start_ind),2)),'s-',num2str(round(time(end_ind),2)),'s'])};
            Mean = all_means;
            SD = all_sd;
            ...
            clear Condition Regressor ROI Seed Uncorrected_pvalue Significant temp_stats T_stats dfs SD Peak_vals all_peak_vals signedrank_wilcoxon zval_wilcoxon p_wilcoxon% HolmBonf_corrected_pvalue
        end
    end
    if makeplot==1
