...
roi = {'ACC_cluster_cope7','insular_cortex_cope7','HB','DRN_sub_PAG_edit_sub_SC_edit_ero'};
...
myreg.parametric = 1;
pre_win = 2;
condition.pre = 'vigilance_firstChecks_constant'; % loads first forage data from pre-disc. phase
...
titles.constant.figure = 'Ai. Switch to check';
titles.parametric.figure = {'Aii. Threat level', 'during switch to check'};
...
colors = 'oranges';
ylims = [-0.4, 0.65];
searchWindow.start = [42]; % beginning of search window for pre-disc. phase peaks.
searchWindow.end = [72]; % end of search window for pre-disc. phase peaks.
...
% Plot
for targetReg = {'constant','parametric'}
