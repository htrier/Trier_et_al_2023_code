...
% Fig. 7D: Switch to forage: Effect of AI*Threat on DRN
roi = {'DRN_sub_PAG_edit_sub_SC_edit_ero'};
seed_roi = {'insular_cortex_cope7'};
titles.figuretitle = [{'Switch to forage:'}, {'Effect of AI*Threat on DRN'}];
titles.figurefile = 'Fig7C_AI-Threat_to_DRN-Forage-only-TC-replication';
titles.statsfile = 'Fig7D_AI-Threat_to_DRN-Forage-only-TC-replication-stats_reproduced';
...
myreg.parametric = 6; % index of parametric regressor of interest
pre_win = 2;
GLM=4;
condition.pre = 'vigilance';
condition.post = 'monitoring';
condition.action = 'forages'; % which action transition to analyze
searchWindow.start = [22]; % if 2s prewindow: [22,72] for [0.0636s, 4.9769s], [4.9769s, 9.9884s]
searchWindow.end = [72]; % if 2s prewindow: [72, 123] for [0.0636s, 4.9769s], [4.9769s, 9.9884s]
twoTailPostTest = 0; % test post-disc. phase with a two-tailed test even if the pre-disc. phase is n.s.
subjects.post = {'sub204','sub109','sub111','sub119','sub121','sub123','sub125',...
    'sub131','sub216','sub224','sub230','sub232'}; % only subjects with <2% effect required for proximity in post-disc. checks
colors='greens';
