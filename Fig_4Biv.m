...
% Effect of AI*time pressure on DRN - switch to forage only (5s pre-window)
roi = {'DRN_sub_PAG_edit_sub_SC_edit_ero'};
seed_roi = {'insular_cortex_cope7'};
tc_action = 'forages'; % if focusing on only one action type, indicate here

% Load behavioral data formatted for Matlab
...
for r = 1:length(roi)
    ...
    for a = 1:length(subjects)
        ...
        if strcmp(cond, 'monitoring_firstChecks_constant')
            seed_TC_checks = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',seed_roi{r},'_monitoring_firstChecks_constant_upsample',num2str(upsample),data_identifier])]);
            seed_TC_forages = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',seed_roi{r},'_monitoring_firstForages_constant_upsample',num2str(upsample),data_identifier])]);
    
            roi_TC_checks  = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',roi{r},'_monitoring_firstChecks_constant_upsample',num2str(upsample),data_identifier])]);
            ...
            seed_TC_checks = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',seed_roi{r},'_',cond,'_upsample',num2str(upsample),data_identifier])]);
            seed_TC_forages = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',seed_roi{r},'_vigilance_firstForages_constant_upsample',num2str(upsample),data_identifier])]);
            
            roi_TC_checks  = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',roi{r},'_',cond,'_upsample',num2str(upsample),data_identifier])]);
            roi_TC_forages  = load([timecourse_dir,subject,'/',feat_name,'/',(['tc_',roi{r},'_vigilance_firstForages_constant_upsample',num2str(upsample),data_identifier])]);
        end
...
%                 end
            ...
            Full_model = {strrep(strjoin(currModel),' ',' + ')}; 
    ...
    if makeplot==1
        sgtitle({strcat("Condition: switch to forages only",' | ROI: ',roi{r},' | Seed: ',seed_roi{r}),strcat('Early win: ',num2str(round(time(start_inds(1)),2)),'s-',num2str(round(time(end_inds(1)),2)),'s , Late win:',num2str(round(time(start_inds(2)),2)),'s-',num2str(round(time(end_inds(2)),2)),' | Test: Wilcoxon | Action coding: [-1,1] (normalized)')},'Interpreter','none');
        pos = get(gcf, 'Position');
        set(gcf, 'Position',pos+[500 0 500 200])
    end
end
% savename=strcat(results_root,'PPI_AIxTimepressure-to-DRN_foragesOnly_reproduced.xlsx');
